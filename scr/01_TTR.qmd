...
corpus <- read.csv("../data/large_poetree_ru_19c_lemma.csv") %>% 
...
Replication of Kjetsaa's 1973 figure: first prepare samples data (three authors, 30k sampled tokens, assigned to 150 samples of 200 words).

```{r, include=TRUE, echo=FALSE}

sample_size <- 200 # number of words added in each step
samples_number <- 150 # for ~30k tokens


# prep corpus data just for three authors
...
# quick check: each author should have 150 samples of 200 words:
corpus_sampled %>% count(sample_x, author) %>% head()
```
...
# quick check
corpus_sampled %>% count(author, sample_x) %>% head()
```
...
```{r, include=FALSE, echo=FALSE}
# check n of tokens in 30k-selection
corpus %>% 
...
TTR in Kjetsaa's norm: read lemmatized corpus, calculate N tokens per author

```{r, include=F, echo=F}
# read the file with Kjetsaa's corpus lemmatized:
norm <- read.csv("../data/kjetsaa_corpus/kjetsaa_norm_lemma.csv")
...
```{r, include=F, echo=F}
# Manually added some random words to have 2k samples for all authors
