...
```{r, include=T, echo=F}
# Kjetsaa's norm corpus replication (full metadata + lemmas)
...
```{r, include=F, echo=T, message=F, warning=F, fig.keep='none', eval=F}
...
```{r, echo=T, include=F, message=F, warning=F, fig.keep='none', eval=F}
...
```{r, include=F, echo=T, message=FALSE, warning=FALSE, fig.keep='none', eval=F}
bar_zr <- zeta_count(AUTHOR = "Baratynskij E.A.",
                         N_POEMS = 245,
                         SAMPLE_SIZE = 38000,
                         NORM = "5k")
...
```{r, include=F, echo=T, warning=F, message=F, fig.keep='none', eval=F}
fet_zr <- zeta_count(AUTHOR = "Fet A.A.",
                        N_POEMS = 600,
                        SAMPLE_SIZE = 36000,
                         NORM = "5k")
...
```{r, include=F, echo=F, eval=F, eval=F}
x <- fet_gr_int %>% 
    mutate(group = "Guiraud\ncoeff.") %>% 
    rbind(fet_lr_int %>% mutate(group = "Weighted\nlog odds\nratio")) %>% 
    rbind(fet_zr_int %>% mutate(group = "Zeta")) %>% 
...
ggsave("../plots/cntrds/hm_kjetsaa.png",
       plot = last_plot(),
       bg = "white", dpi = 300,
       width = 15, height = 10)
```
...
# ggsave("../plots/cntrds/listsim_5k.png", 
#        plot = last_plot(), 
#        bg = "white", dpi = 300,
#        width = 8, height = 6)
```
...
ggsave("../plots/cntrds/hm_random.png",
       plot = last_plot(),
       bg = "white", dpi = 300,
       width = 15, height = 10)
```
